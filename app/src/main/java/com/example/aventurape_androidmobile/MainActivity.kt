        ...
        setContent {
            AventuraPe_AndroidMobileTheme {
                Scaffold( modifier = Modifier.fillMaxSize() ) {
                        innerPadding -> MainScreen(outerPadding = innerPadding) // <-- Pasa innerPadding aquí
                }
            }
        }
    }
...
@Composable
fun MainScreen(outerPadding: PaddingValues) { // <-- Agrega este parámetro
    ...
    ) { innerScaffoldPadding -> // <-- Renombra este padding para claridad
        Box(
            // Combina el padding externo y el interno
            modifier = Modifier
                .padding(outerPadding) // <-- Aplica el padding del Scaffold exterior
                .padding(innerScaffoldPadding) // <-- Aplica el padding del Scaffold interior
                .fillMaxSize()
        ) {
